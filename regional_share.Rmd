...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, echo = FALSE, cache = TRUE)
...
zones_land <- read_rds("K:\\Projects\\OrangeCoNC\\Features\\R\\data\\zones_land.rds")

corridor <- read_rds("K:\\Projects\\OrangeCoNC\\Features\\R\\data\\corridor.rds")

station_buffers <- read_rds("K:\\Projects\\OrangeCoNC\\Features\\R\\data\\station_buffers.rds")

corridor_buffer_half <- read_rds("K:\\Projects\\OrangeCoNC\\Features\\R\\data\\corridor_buffer_half.rds")

station_points <- read_rds("K:\\Projects\\OrangeCoNC\\Features\\R\\data\\station_points.rds")
```

The following charts reveal the distribution of foretasted dwelling unit and employment growth between the corridor and non-corridor area. They reveal the share of regional growth expected to occur inside and outside the corridor. A "target" scenario was then developed to compare what this growth would look like if growth were concentrated in the corridor in order to reach the TOD BRT target.
...
corridor_growth_emp_target_scenario <- (area_weighted_corridor_results_EMP$acres*13)-(area_weighted_corridor_results_EMP$TotEmp2020)
...
#growth of corridor to hit target
corridor_growth_du_target_scenario <- (area_weighted_corridor_results_DU$acres*13)-(area_weighted_corridor_results_DU$Tot_DU_20)
