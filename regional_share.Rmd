...
area_weighted_summary_wider <- corridor_area_weighted_combined %>% pivot_wider(names_from = year, 
                                                                               values_from = count, names_prefix = "yr_") %>% 
...
```{r}
kbl(area_weighted_summary_wider)
```



**Note: This analysis assumes new DU and EMP allocated to corridor TAZs are equally spread by area, meaning the corridor estimate may be slightly lower than if all of these units were allocated only within the corridor portion of the TAZ.**